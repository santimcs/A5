...
begin
  ...
  loop do
    ...
    begin
      ...
      # Calculate and display transaction info
      if market_volume != "N/A" && last_price != "N/A"
        current_volume = market_volume.gsub(',', '').to_i
        if previous_volume
          volume_diff = current_volume - previous_volume
          transaction_type = last_price == bid_price ? 'Bid' : 'Offer'
          puts "Last Trans:  #{transaction_type.ljust(5)} | #{volume_diff.to_s.reverse.gsub(/(\d{3})(?=\d)/, '\1,').reverse}"
        end
