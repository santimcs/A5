...
Promise.all(videoDetailsPromises).then(videosWithDetails => {
  ...
  console.log(`Title: ${title}`); // Log the title for debugging
    ...
    // Log each character and its char code
    ...
    // Debugging with an exact title (replace 'Exact Title Here' with an actual title from your list)
    // const filterString = '[中日字幕]'.trim();
    // const isFilteredTitle = title === filterString;

    // console.log(`[中日字幕]`); // Log the result of the title comparison
    // console.log(`Filtered by title (${title}): ${isFilteredTitle}`);
    ...
    if (totalSeconds >= 300 && !isFilteredTitle) {
      const videoRow = document.createElement('tr');
      videoRow.innerHTML = `
        <td class="videoThumbnail"><img src="${thumbnail}" alt="${title}" /></td>
        <td>${title}</td>
        ...
        <td>${duration}</td>
        <td>${views}</td>
        <td>${totalSeconds}</td> <!-- Added total seconds cell -->
      `;
      videoRow.addEventListener('click', () => {
        playVideo(videoId);
      });
