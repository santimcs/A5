...
async function loadVideoData() {
  try {
    // Use absolute path to CSV file in the same S3 folder
    const csvUrl = 'yt_videos.csv';
    
    // Fetch the CSV file from the same directory
    const csvResponse = await fetch(csvUrl);
    if (!csvResponse.ok) {
      throw new Error(`Failed to load CSV file: ${csvResponse.status} ${csvResponse.statusText}`);
    }
    
    const csvText = await csvResponse.text();
    const videoIds = parseCSV(csvText);
